"""
COMPLETE RAG-LLM SYSTEM - ALL COMPONENTS INCLUDED
==================================================

This file contains EVERYTHING you need:
1. Document Indexer (Embedder)
2. Vector Database (Retriever)  
3. LLM Interface (Generator)
...
Usage:
    python complete_rag_llm_system.py
"""
...
class Embedder:
    ...
    This enables semantic search - finding similar meanings, not just keywords.
    """
    ...
    def __init__(self, model_name: str = "all-MiniLM-L6-v2"):
        ...
        Args:
            model_name: Name of the sentence-transformer model
                       Default is small, fast, and good quality
        """
...
# ============================================================================
# 2. INDEXER - Processes and stores documents
# ============================================================================

class DocumentIndexer:
    """
    Indexes PDF documents for fast retrieval.
    
    Workflow:
    1. Extract text from PDFs
    2. Split into chunks
    3. Generate embeddings
    4. Store for searching
    """
    ...
    def index_directory(self, pdf_directory: str) -> bool:
        ...
        for idx, pdf_path in enumerate(pdf_files, 1):
            ...
            print(f"    ✓ {len(page_texts)} pages processed")
    ...
    def save_index(self, filepath: str):
        ...
        print(f"💾 Index saved: {filepath} ({file_size:.2f} MB)")
...
# ============================================================================
# 3. RETRIEVER - Searches for relevant content
# ============================================================================
...
class Retriever:
    ...
    def __init__(self, indexer: DocumentIndexer, embedder: Embedder):
        ...
        Args:
            indexer: DocumentIndexer with indexed documents
...
# ============================================================================
# 4. LLM GENERATOR - Generates answers using LLM
# ============================================================================

class LLMGenerator:
    """
    Generates natural language answers using various LLM backends.
    
    Supports:
    - Ollama (local, private)
    - Anthropic Claude (cloud, best quality)
    - OpenAI GPT-4 (cloud)
    - Template mode (no LLM, extraction only)
    """
    ...
    def __init__(self, mode: str = "local"):
        ...
        Args:
            mode: "local" (Ollama), "anthropic", "openai", or "template"
        """
        self.mode = mode
        self.client = None
    ...
    def generate(self, query: str, context: str, max_tokens: int = 1000) -> str:
        ...
        Returns:
            Generated answer
        """
        prompt = f"""Based on the following context from technical documents, answer the question accurately.
...
Provide a clear, specific answer. Cite page numbers when possible."""
    ...
    def _generate_template(self, query: str, context: str) -> str:
        """Generate answer by extracting from context (no LLM)."""
        ...
        if relevant:
            ...
            return "Relevant information found in context. Please review the sources."


# ============================================================================
# 5. RAG PIPELINE - Puts it all together
# ============================================================================
...
class RAGPipeline:
    ...
    2. Indexer (process & store documents)
    3. Retriever (search for relevant content)
    4. Generator (create natural language answers)
    """
    ...
    def __init__(self, mode: str = "local"):
        ...
        Args:
            mode: LLM mode ("local", "anthropic", "openai", "template")
        """
        print(f"\n{'='*70}")
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            print("  2. Anthropic Claude (Best quality, API key needed)")
            print("  3. OpenAI GPT-4 (Good quality, API key needed)")
            ...
            if not rag:
                print("\n⚠️ Please choose LLM mode first (option 1)")
                continue
            
            pdf_dir = input("\n📁 Enter PDF directory path: ").strip()
            if pdf_dir:
            ...
            if not rag:
                print("\n⚠️ Please initialize system first (option 1)")
                continue
            
            if not rag.indexer.chunks:
                print("\n⚠️ Please index documents first (option 2)")
                continue
            ...
            if not rag or not rag.indexer.chunks:
                print("\n⚠️ Please index documents first")
                continue
            ...
            filename = input("\n💾 Enter filename: ").strip() or "rag_index.json"
            ...
            print(f"  Chunks indexed: {len(rag.indexer.chunks)}")
            print(f"  Embedding dimension: {rag.embedder.dimension}")
